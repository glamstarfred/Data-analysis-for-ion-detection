...
def flux_plot(df):
    ...
    for group_number, group_df in grouped_df:
        ...
        df_sum2['Step Number'] = range(0,64)
    ...
    for channel in channels:
        ...
        # color setting
        im = ax.imshow(df_spec.values, norm=colors.LogNorm(vmin=1e+1, vmax=df_spec.values.max()),cmap='jet')

        ax.set_xticks(np.linspace(0, len(df_spec.columns)-1, 4))
        ax.set_yticks(np.linspace(0, len(df_spec.index)-1, 4))
